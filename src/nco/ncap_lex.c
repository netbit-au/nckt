...
#line 165 "ncap_lex.l"
{
  ...
  if(ncap_ncl_dpt_crr >= NCO_MAX_NCL_DPT){
    (void)fprintf(stderr,"%s: ERROR File %s nesting depth of %lu is too deep",prg_nm_get(),nco_yytext,(unsigned long)ncap_ncl_dpt_crr);
    nco_exit(EXIT_FAILURE);
  } /* endif */
  if(dbg_lvl_get() > 1) (void)fprintf(stderr,"%s: INFO Will descend from file %s at nesting depth %lu to #include'd file %s at depth %lu\n",prg_nm_get(),ncap_fl_spt_glb[ncap_ncl_dpt_crr],(unsigned long)ncap_ncl_dpt_crr,nco_yytext,(unsigned long)ncap_ncl_dpt_crr+1UL);
...
#line 302 "ncap_lex.l"
{
  ...
  lval_ptr->scv.val.d=strtod(nco_yytext,(char **)NULL);
...
nco_yywrap(void)
{
  ...
  if(dbg_lvl_get() >= 3) (void)fprintf(stderr,"%s: DEBUG nco_yywrap() called with ncap_ncl_dpt_crr = %lu\n",prg_nm_get(),(unsigned long)ncap_ncl_dpt_crr);
