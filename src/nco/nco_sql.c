...
PGconn *nco_psql(char *host,char *user,char *dbname);

void *
nco_sql_rec_clc /* [fnc] Multi-slab algorithm (recursive routine, returns a single slab pointer */
 ...
 var_sct *vara) /* [sct] Information for routine to read variable information and pass information between calls */
{
  ...
  if(nbr_slb > 1){
	...
	slb_sz=(ptrdiff_t)(lcnt*(lmt_lst[dpt_crr]->lmt_dmn[slb_idx]->cnt));
      ...
      while(nco_sql_clc_idx(True,lmt_lst[dpt_crr],&indices[0],&lmt_ret,&slb_idx)){
	cp_stp=(char *)vp+cp_fst;
...
void
nco_sql_prn_idx(lmt_all_sct *lmt_i)
{
  int slb_nbr;
  int idx;
  ...
  while(nco_sql_clc_idx(False,lmt_i,&indices[0],&lmt,&slb_nbr))
...
void
nco_sql_ram_2_dsk /* convert hyperslab indices (in RAM) to hyperlsab indices relative */
...
void
nco_sql_wrp_splt /* [fnc] Split wrapped dimensions */
(lmt_all_sct *lmt_lst)
{
  int idx;
  int jdx;
...
} /* end nco_sql_wrp_splt() */

void
nco_sql_var_get    /* [fnc] Get var data from disk taking accound of multihyperslabs */
...
/* Initialize lmt_mult with multi-limits from lmt_lst limits */
  ...
  void_ptr=nco_sql_rec_clc(0,nbr_dim,lmt,lmt_mult,var_in);
...
void
nco_sql_c_2_f /* [fnc] Replace brackets with parentheses in a string */
...
void nco_sql_ins_var_val   /* [fnc] Print variable data */
(PGconn *conn,
 char * pgschema,
 char * pgtable,
 const int in_id, /* I [id] netCDF input file ID */
 ...
 const nco_bool PRN_DMN_VAR_NM) /* I [flg] Print dimension/variable names */
{
  ...
  (void) fprintf(stderr, "Insert into %s.%s\n", pgschema, pgtable);
  exit(1);
  ...
  if(var.nbr_dim > 0 && dlm_sng == NULL){
	...
	dmn_sbs_ram[idx]=(lmn%mod_map_out[idx])/(idx == var.nbr_dim-1 ? 1L : mod_map_out[idx+1]);
      ...
      (void)nco_sql_ram_2_dsk(dmn_sbs_ram,lmt_mult,var.nbr_dim,dmn_sbs_dsk,(lmn==var.sz-1));
	...
	for(idx=0;idx<var.nbr_dim;idx++){
	  ...
	  if(PRN_DMN_VAR_NM) (void)sprintf(dmn_sng,"thread_1 %%s[%%ld]=%s ",nco_typ_fmt_sng(dim[dmn_idx].type)); else (void)sprintf(dmn_sng,"%s ",nco_typ_fmt_sng(dim[dmn_idx].type));
          dmn_sbs_prn=dmn_sbs_dsk[dmn_idx];

	  if(FORTRAN_IDX_CNV){
            (void)nco_sql_c_2_f(dmn_sng);
	    dmn_sbs_prn++;
	  } /* end if */
	...
	continue;
      ...
      if(PRN_DMN_VAR_NM) (void)sprintf(var_sng,"4 %%s[%%ld]=%s %%s\n",nco_typ_fmt_sng(var.type)); else (void)sprintf(var_sng,"%s\n",nco_typ_fmt_sng(var.type));
      if(FORTRAN_IDX_CNV){
	(void)nco_sql_c_2_f(var_sng);
	var_dsk++;
 ...
 int lmt_nbr)
{
  ...
  for(idx=0;idx<nbr_dmn_fl;idx++){
    ...
    (void)nco_sql_wrp_splt(lmt_all_lst[idx]);
    ...
    if(lmt_all_lst[idx]->WRP == True){
      ...
      (void)nco_sql_clc_cnt(lmt_all_lst[idx]);
      continue;
    } /* endif */
    ...
    /* Sort limits */
    (void)nco_sql_qsort_srt(lmt_all_lst[idx]);
    /* Check for overlap */
    bovl=nco_sql_ovl(lmt_all_lst[idx]);
...
} /* end nco_sql_lmt_all_int() */

void
nco_sql_var_val_cpy /* [fnc] Copy variables data from input to output file */
(const int in_id, /* I [enm] netCDF file ID */
 ...
 int nbr_dmn_fl) /* I [nbr] Number of multi-hyperslab limits */
{
  /* NB: nco_sql_var_val_cpy() contains OpenMP critical region */
  ...
  if(nbr_dim==0){
   ...
   (void)nco_get_var1(in_id,var[idx]->id,0L,var[idx]->val.vp,var[idx]->type);
    ...
    /* Call super-dooper recursive routine */
    /* NB: nco_sql_rec_clc() with same nc_id contains OpenMP critical region */
    var[idx]->val.vp=nco_sql_rec_clc(0,nbr_dim,lmt,lmt_mult,var[idx]);

    (void)nco_free(lmt_mult);
    (void)nco_free(lmt);
   } /* end else nbr_dim */

    /*(void)nco_sql_var_get(in_id,var[idx],lmt_lst,nbr_dmn_fl); */
